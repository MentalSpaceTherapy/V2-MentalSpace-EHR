...
async function seedLeads(
  ...
  if (existingLeads.length === 0) {
    ...
    for (let i = 0; i < 20; i++) {
      ...
      // Assign sources based on patterns - explicitly typed as string | null
      if (i < 5) {
        source = psychologyTodaySource ? ("Psychology Today" as const) : null;
        sourceId = psychologyTodaySource ? (psychologyTodaySource.id as number) : null;
      } else if (i < 10) {
        source = googleAdsSource ? ("Google Ads" as const) : null;
        sourceId = googleAdsSource ? (googleAdsSource.id as number) : null;
        marketingCampaignId = googleAdsCampaign ? (googleAdsCampaign.id as number) : null;
      } else if (i < 15) {
        source = drJohnsonSource ? ("Professional Referral" as const) : null;
        sourceId = drJohnsonSource ? (drJohnsonSource.id as number) : null;
      } else {
        source = communityHealthSource ? ("Community Outreach" as const) : null;
        sourceId = communityHealthSource ? (communityHealthSource.id as number) : null;
        marketingCampaignId = workshopCampaign ? (workshopCampaign.id as number) : null;
      }
